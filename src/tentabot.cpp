...
void Tentabot::setProcessParams(Tentabot::ProcessParams new_process_param)
{
  ...
  process_param.ground_collision_flag = new_process_param.ground_collision_flag;
  process_param.ground_collision_threshold = new_process_param.ground_collision_threshold;
  process_param.drl_service_flag = new_process_param.drl_service_flag;
}

void Tentabot::setRobotParams(Tentabot::RobotParams new_robot_param)
{
  robot_param.world_name = new_robot_param.world_name;
  robot_param.local_map_msg = new_robot_param.local_map_msg;
  ...
  robot_param.robot_odometry_msg = new_robot_param.robot_odometry_msg;
}
...
void Tentabot::setOnTuningParams(Tentabot::OnTuningParams new_on_tuning_param)
{
  ...
  on_tuning_param.occupancy_scale = new_on_tuning_param.occupancy_scale;
...
void Tentabot::setStatusParams(Tentabot::StatusParams new_status_param)
{
  ...
  status_param.measured_local_map = new_status_param.measured_local_map;
...
void Tentabot::setDataPath(string new_data_path)
{
...
void Tentabot::robotPoseCallback(const geometry_msgs::Pose::ConstPtr& msg)
{
  ...
  if(process_param.ground_collision_flag && status_param.measured_robot_pose.position.z < process_param.ground_collision_threshold)
  {
    //cout << "Tentabot::robotPoseCallback -> Did I fall?" << endl;
...
void Tentabot::odometryCallback(const nav_msgs::Odometry& msg)
{
  ...
  if(process_param.ground_collision_flag && status_param.measured_robot_pose.position.z < process_param.ground_collision_threshold)
  {
    //cout << "Tentabot::odometryCallback -> Did I fall?" << endl;
    status_param.nav_result = -1;
  }
}
...
void Tentabot::mainCallback(const ros::TimerEvent& e) 
{
  ...
  else
  {
    if (data_path != "")
    {
      ...
      string post_data_filename = data_directory + data_tag + "_result_" + robot_param.world_name + ".csv";
    ...
    cout << "Tentabot::mainCallback -> nav_result: " << status_param.nav_result << endl;
    cout << "Tentabot::mainCallback -> nav_duration: " << status_param.nav_duration << endl;
    cout << "Tentabot::mainCallback -> nav_length: " << status_param.nav_length << endl;

    ros::shutdown();
  }
}

bool Tentabot::rl_step_srv(tentabot::rl_step::Request &req, tentabot::rl_step::Response &res) 
{
  // UPDATE PLANNING STATES
  update_planning_states();
...
bool Tentabot::update_goal_srv(tentabot::update_goal::Request &req, tentabot::update_goal::Response &res)
{
  goal_util.setGoalPoint(req.goal);

  cout << "Tentabot::update_goal_srv -> Goal updated to x: " << goal_util.getActiveGoal().position.x << ", y: " << goal_util.getActiveGoal().position.y << endl;
...
void Tentabot::initialize(NodeHandle& nh)
{
  ...
  status_param.ex_best_tentacle = 0;
  status_param.ex_best_sample = 0;
  ...
  //ros::Subscriber sub_odom = nh.subscribe(rp.robot_odometry_msg, 1000, &Tentabot::robotPoseCallback, &tbot);
  ...
  srv_update_goal = nh.advertiseService("update_goal", &Tentabot::update_goal_srv, this);
  ...
  if (data_path != "")
  {
    ...
    input_data_stream << "occupancy_scale," << on_tuning_param.occupancy_scale << "\n";
    ...
    string pre_data_filename = data_directory + data_tag + "_pre_" + robot_param.world_name + ".csv";
    ...
    string mid_data_filename = data_directory + data_tag + "_process_" + robot_param.world_name + ".csv";
...
void Tentabot::update_heuristic_values()
{
  ...
  int n_close = 5;
  vector<int> vsamp;
  tf::Vector3 crash_po_wrt_robot;
  tf::Vector3 crash_po_wrt_world;
  ...
  for(int k = 0; k < tentacle_cnt; k++)
  {
    ...
    // DETERMINE TARGET CLOSENESS AND SMOOTHNESS VALUES
    double total_dist_close = 0;
    double total_dist_smooth = 0;
    subsample(vsamp, n_close);

    for (int i = 0; i < vsamp.size(); ++i)
    {
      // DETERMINE TARGET CLOSENESS VALUE
      crash_po_wrt_robot.setX(off_tuning_param.tentacle_data[k][vsamp[i]].x);
      crash_po_wrt_robot.setY(off_tuning_param.tentacle_data[k][vsamp[i]].y);
      crash_po_wrt_robot.setZ(off_tuning_param.tentacle_data[k][vsamp[i]].z);

      crash_po_wrt_world = status_param.transform_robot_wrt_world * crash_po_wrt_robot;
      total_dist_close += find_Euclidean_distance(active_goal.position, crash_po_wrt_world);
...
//            total_dist_smooth += find_Euclidean_distance(ex_best_velocity, k_velocity);
      ...
      {
        cout << "Tentabot::update_heuristic_values -> Error: Tentacle index!" << endl;
      }
    }
    heuristic_param.closeness_set[k] = total_dist_close / n_close;
    heuristic_param.smoothness_set[k] = total_dist_smooth / n_close;
  }
  //fillDebugArrayVisu(v, robot_param.robot_frame_name);
  //publishDebugArrayVisu();

  normalize(heuristic_param.closeness_set);
  normalize(heuristic_param.smoothness_set);
}
...
void Tentabot::select_best_tentacle()
{
  ...
  double weighted_occupancy_value;
  ...
  for(int k = 0; k < tentacle_cnt; k++)
  {
    if( heuristic_param.navigability_set[k] != 0 )
    {
      ...
      weighted_occupancy_value = on_tuning_param.occupancy_scale * heuristic_param.occupancy_set[k];
      ...
      tentacle_value = weighted_occupancy_value + weighted_closeness_value + weighted_smoothness_value;
      ...
      cout << "" << endl;
      ROS_INFO_STREAM("occupancy_scale: " << on_tuning_param.occupancy_scale);
      ROS_INFO_STREAM("occupancy: " << heuristic_param.occupancy_set[k]);
      ROS_INFO_STREAM("weighted_occupancy_value: " << weighted_occupancy_value);
      cout << "" << endl;
      ...
      */
      ...
      {
        if(tentacle_value < best_tentacle_value)
        {
          best_tentacle_value = tentacle_value;
          status_param.best_tentacle = k;   
        }
...
void Tentabot::send_motion_command_by_pose_control()
{
  ...
  if(!status_param.navexit_flag)
  {
    ...
    if (status_param.counter <= 1)
    {
      status_param.command_pose.pose = status_param.robot_pose;
    }
    else
    {
      ...
      if(dist2goal < dist2goal_threshold || abs(next_yaw) > next_yaw_threshold)
      {
      ...
      if (status_param.desired_lat_speed > status_param.lat_speed)
      {
        status_param.lat_speed_weight += weight_inc_dec;
      }
      else if (status_param.desired_lat_speed < status_param.lat_speed)
      {
        status_param.lat_speed_weight -= weight_inc_dec;
      }

      // CHECK THE LATERAL SPEED WEIGHT BOUNDARIES
      if (status_param.lat_speed_weight < min_lat_speed_weight)
      {
        status_param.lat_speed_weight = min_lat_speed_weight;
      }

      if (status_param.lat_speed_weight > max_lat_speed_weight)
      {
        status_param.lat_speed_weight = max_lat_speed_weight;
      }
...
void Tentabot::send_motion_command_by_velocity_control()
{
  ...
  else if(dist2goal < process_param.goal_close_threshold)   // reach goal
  {
    if(!goal_util.switchActiveGoal())
    {
      cout << "Tentabot::send_motion_command_by_velocity_control -> Cawabunga! The goal has reached!" << endl;
      status_param.navexit_flag = true;
      status_param.nav_result = 1;
    }
    else
    {
      cout << "Tentabot::send_motion_command_by_velocity_control -> Yey! Waypoint #" << goal_util.getActiveGoalIndex()-1 << " has reached!" << endl;
      active_goal = goal_util.getActiveGoal();
      dist2goal = find_Euclidean_distance(active_goal.position, status_param.robot_pose.position);
    }
  }
  ...
  {
    cout << "Tentabot::send_motion_command_by_velocity_control -> Ugh! I am too late..." << endl;
    status_param.navexit_flag = true;
    status_param.nav_result = 0;
  }
