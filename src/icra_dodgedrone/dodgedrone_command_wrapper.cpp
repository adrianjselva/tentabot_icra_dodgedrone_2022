...
DodgeDroneCommandWrapper::DodgeDroneCommandWrapper(ros::NodeHandle& n) : nh_(n), quadrotor(0.1265158) {
    ...
    this->command_publisher = nh_.advertise<geometry_msgs::TwistStamped>("/kingfisher/dodgeros_pilot/velocity_command", 1);
    ...
    nh_.getParam("xKp", xKp);
    nh_.getParam("xKi", xKi);
    nh_.getParam("xKd", xKd);

    nh_.getParam("yKp", yKp);
    nh_.getParam("yKi", yKi);
    nh_.getParam("yKd", yKd);

    nh_.getParam("zKp", zKp);
    nh_.getParam("zKi", zKi);
    nh_.getParam("zKd", zKd);

    nh_.getParam("yawKp", yawKp);
    nh_.getParam("yawKi", yawKi);
    nh_.getParam("yawKd", yawKd);
...
//    this->z = PID(zKp, zKi, zKd, 100000, 0.1);
    ...
    this->x = PID(xKp, xKi, xKd, 10000, 0.01);
    this->y = PID(yKp, yKi, yKd, 10000, 0.01);
    this->z = PID(zKp, zKi, yKd, 10000, 0.01);
    this->yaw = PID(yawKp, yawKi, yawKd, 100000, 0.01);
}

void DodgeDroneCommandWrapper::transformVelocity(const geometry_msgs::TwistStampedConstPtr &vel) {
    double xVelocity = this->x.calculate(vel->twist.linear.x, this->quad_state->velocity.linear.x);
    double yVelocity = this->y.calculate(vel->twist.linear.y, this->quad_state->velocity.linear.y);
    double zVelocity = this->z.calculate(vel->twist.linear.z, this->quad_state->velocity.linear.z);
    double yawVelocity = this->yaw.calculate(vel->twist.angular.z, this->quad_state->velocity.angular.z);

    auto velocityCommand = geometry_msgs::TwistStampedPtr(new geometry_msgs::TwistStamped);
    velocityCommand->twist.linear.x = xVelocity;
    velocityCommand->twist.linear.y = yVelocity;
    velocityCommand->twist.linear.z = zVelocity;
    velocityCommand->twist.angular.z = yawVelocity;

    this->command_publisher.publish(vel);
}
